# cinaR [![](reference/figures/cinaR.png)](https://eonurk.github.io/cinaR/)
...
Use
[`prep_scATAC_cinaR()`](https://eonurk.github.io/cinaR/reference/prep_scATAC_cinaR.md)
to pseudobulk 10x scATAC peak-by-cell matrices into a cinaR-ready
consensus matrix.  
...
If your peak IDs are not in `chr:start-end`, `chr_start_end`, or
`chr-start-end` format, pass a `peak.bed` data.frame with `CHR`,
`START`, and `STOP` columns via `peak.bed = ...`.
